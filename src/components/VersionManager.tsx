...
const VersionManager: React.FC<VersionManagerProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 dark:bg-black/80 backdrop-blur-md z-100 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[80vh] transition-colors">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <button
            onClick={handleSave}
            disabled={isSaving}
            className="w-full flex items-center justify-center gap-2 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 border-dashed text-slate-600 dark:text-slate-300 py-4 rounded-xl text-sm font-bold transition-all mb-6 group"
          >
            {isSaving ? (
          ...
          <div className="space-y-3">
            {isLoading && versions.length === 0 ? (
              ...
              <div className="py-12 flex flex-col items-center gap-4 text-slate-400 dark:text-slate-500 border border-slate-200 dark:border-slate-800 border-dashed rounded-2xl">
              ...
              versions.map((version) => (
                <div
                  key={version.id}
                  className="group bg-white dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-600 rounded-xl p-4 flex items-center gap-4 transition-all"
                >
                  <div className="bg-slate-100 dark:bg-slate-800 p-2.5 rounded-lg text-slate-500 dark:text-slate-400 group-hover:text-purple-500 dark:group-hover:text-purple-400 transition-colors">
                    <FileText size={20} />
                  </div>
                    ...
                    {editingId === version.id ? (
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          className="flex-1 bg-white dark:bg-slate-900 border border-purple-500/50 rounded px-2 py-1 text-sm text-slate-900 dark:text-white focus:outline-none"
                        />
                        <button 
                      ...
                      <>
                        <div className="flex items-center gap-2">
                          <h3 className="text-sm font-bold text-slate-900 dark:text-slate-200 truncate">
                            {version.name}
                          </h3>
                          <button
                            onClick={() => startEditing(version)}
                            className="opacity-0 group-hover:opacity-100 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 transition-all"
                          >
                            <Pencil size={12} />
                          </button>
                        </div>
                  ...
                  <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all">
                    ...
                    <button
                      ...
                      className="p-2 text-slate-400 dark:text-slate-600 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-400/10 rounded-lg transition-all"
                    >
                      <Trash2 size={16} />
                    </button>
                  </div>
