...
const VersionManager: React.FC<VersionManagerProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 dark:bg-black/80 backdrop-blur-md z-100 flex items-center justify-center p-4">
      <div className="bg-surface-bg border border-border-base rounded w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[80vh] transition-colors duration-300">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <button
            ...
            onClick={handleSave}
            disabled={isSaving}
            className="w-full flex items-center justify-center gap-2 bg-app-bg hover:bg-slate-100 dark:hover:bg-slate-700 border border-border-base border-dashed text-text-muted py-4 rounded text-sm font-bold transition-all mb-6 group"
          >
            {isSaving ? (
          ...
          <div className="space-y-3">
            {isLoading && versions.length === 0 ? (
              ...
              <div className="py-12 flex flex-col items-center gap-4 text-text-muted border border-border-base border-dashed rounded">
              ...
              versions.map((version) => (
                <div
                  key={version.id}
                  className="group bg-surface-bg border border-border-base hover:border-slate-300 dark:hover:border-slate-600 rounded p-4 flex items-center gap-4 transition-all"
                >
                  <div className="bg-app-bg p-2.5 rounded text-text-muted group-hover:text-purple-500 dark:group-hover:text-purple-400 transition-colors">
                    <FileText size={20} />
                  </div>
                    ...
                    {editingId === version.id ? (
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          onKeyDown={(e) =>
                            e.key === "Enter" && handleRename(version.id)
                          }
                  ...
                  <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all">
                    <button
                      ...
                      className="bg-purple-600/10 hover:bg-purple-600 text-purple-600 dark:text-purple-400 hover:text-white px-3 py-1.5 rounded text-[10px] font-black transition-all"
                    >
                      LOAD
                    </button>
                    <button
                      ...
                      className="p-2 text-text-muted hover:text-red-600 dark:hover:text-red-400 hover:bg-red-400/10 rounded transition-all"
                    >
                      <Trash2 size={16} />
                    </button>
                  </div>
        ...
        <div className="px-6 py-4 bg-surface-bg border-t border-border-base">
          <p className="text-[10px] text-text-muted text-center leading-relaxed italic">
            Versions are unique to this browser. Clear your cache or use a
            different browser and they won't be visible.
          </p>
        </div>
      </div>
    </div>
