import { Key, Save, ShieldCheck, Trash2, X } from "lucide-react";
import type React from "react";
import { useEffect, useState } from "react";
import { clearAIConfig, getAIConfig, saveAIConfig } from "../utils/aiService";
...
const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 dark:bg-black/80 backdrop-blur-md z-100 flex items-center justify-center p-4">
      <div className="bg-surface-bg border border-border-base rounded w-full max-w-md overflow-hidden shadow-2xl flex flex-col transition-colors duration-300">
        {/* Header */}
        ...
        <div className="p-6 space-y-6">
          <div className="bg-emerald-500/10 border border-emerald-500/20 rounded p-4 flex gap-3">
            <ShieldCheck
              size={20}
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <input
                ...
                placeholder="sk-..."
                className="w-full bg-white dark:bg-slate-950 border border-border-base rounded px-4 py-2.5 text-sm text-text-main focus:border-emerald-500 outline-none transition-all"
              />
            </div>
        ...
        <div className="px-6 py-4 bg-surface-bg border-t border-border-base flex justify-between items-center">
          ...
          <button
            type="button"
            onClick={handleSave}
            className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white px-5 py-2 rounded text-sm font-bold transition-all shadow-lg active:scale-95"
          >
            {showSaved ? (
